...
% in front of p-pd term in cost function. Keep them one because I'm not
...
title('$\bar{p}$', 'Interpreter', 'latex')
xlabel('x')
ylabel('t')
zlabel('p')

figure(6)
surf(X_full,T_full,ud, 'EdgeColor','none')
hold on
for i = 1 : spacing : params.nx
    plot3(X_full(:,i), T_full(:,i), ud(:,i),'-k');
    plot3(X_full(i,:), T_full(i,:), ud(i,:),'-k');
end
...
figure(7)
surf(X,T,pd, 'EdgeColor','none')
hold on
for i = 1 : spacing : params.nx
    plot3(X(:,i), T(:,i), pd(:,i),'-k');
    plot3(X(i,:), T(i,:), pd(i,:),'-k');
end
